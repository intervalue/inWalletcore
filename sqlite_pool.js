...
module.exports = function (db_name, MAX_CONNECTIONS, bReadOnly) {
    ...
    async function connectSync() {
        ...
        await connection.query("PRAGMA synchronous=FULL");
    ...
    function takeConnectionFromPool(handleConnection) {
        ...
        // add by pmj can refree connection when is out than 5s
        for (var i = 0; i < arrConnections.length; i++){
            // if (new Date().getTime() - arrConnections[i].time > 5000){
            //     //console.log('timeout sql ----------------------');
            //     console.log(arrConnections[i].sql);
            //     arrConnections[i].release();
            // }
            if (!arrConnections[i].bInUse) {
                //console.log("reusing previously opened connection");
                arrConnections[i].bInUse = true;
                arrConnections[i].time = new Date().getTime();
                return handleConnection(arrConnections[i]);
            }
        }

        // second, try to open a new connection
        //console.log('arrConnections.length: ',arrConnections[0])
        //console.log('arrConnectionSyncs.length: ',arrConnectionSyncs.length)
    ...
    async function takeConnectionFromPoolSync() {
        ...
        if(arrConnectionSyncs.length == 0){
            connection = await connectSync();
            arrConnectionSyncs.push(connection);
        }else connection = arrConnectionSyncs[0];
        // console.log('takeConnectionFromPoolSync: ',arrConnectionSyncs)
        return connection;
    }
