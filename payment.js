...
let buildCallData = async (data,cb) => {
    try{
        let res = utils.PrefixInteger(data.method.toString(16),64)+utils.PrefixInteger(data.timeTerm.toString(16),64)+utils.PrefixInteger(data.interestRate.toString(16),64);
        if(cb) cb(null,res);
        else return res;
    }catch (e) {
